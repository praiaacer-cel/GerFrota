...
fun RelatorioAcertoMotoristaScreen() {
    ...
    var mes by remember {
        mutableStateOf(java.util.Calendar.getInstance().get(java.util.Calendar.MONTH) + 1)
    }
    var ano by remember {
        mutableStateOf(java.util.Calendar.getInstance().get(java.util.Calendar.YEAR))
    }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Acerto de Motorista") }) }) { pad ->
        Column(Modifier.padding(pad).padding(16.dp)) {
            ...
            ac?.let { a ->
                ...
                Text(
                    if (a.saldo >= 0) {
                        "A frota deve pagar ${DatabaseHelper.fmtBRL(a.saldo)} ao motorista."
                    } else {
                        "O motorista está devendo ${DatabaseHelper.fmtBRL(abs(a.saldo))}."
                    },
