        ...
        setContent {
            GerFrotaTheme {
                ...
                NavHost(nav, if (prefs.getBoolean("logado", false)) "dashboard" else "login") {
                    ...
                    composable("manutencao_form/{placa}/{tipo}/{manutId}/{pneuId}") { b -> val resultadoPneu = b.savedStateHandle?.get<String>("resultado_pneu"); ManutencaoFormScreen(b.arguments?.getString("placa") ?: "", dec(b.arguments?.getString("tipo") ?: ""), b.arguments?.getLong("manutId") ?: -1, b.arguments?.getLong("pneuId") ?: -1, resultadoPneu, { b.savedStateHandle?.remove("resultado_pneu") }, { nav.popBackStack() }) }
                    composable("pneus_gestao/{placa}/{tipo}") { b -> val placa = b.arguments?.getString("placa") ?: ""; val tipo = dec(b.arguments?.getString("tipo") ?: ""); PneusGestaoScreen(placa, tipo, onMap = { nav.navigate("pneus_map/$placa/${enc(tipo)}/0") }, onServicos = { nav.navigate("pneus_servicos/$placa") }, onRodizio = { nav.navigate("pneus_rodizio/$placa/${enc(tipo)}") }, onEstoque = { nav.navigate("pneus_estoque/$placa/${enc(tipo)}") }, onBack = { nav.popBackStack() }) }
                    ...
                    composable("combustivel_menu/{placa}/{veiculoId}") { b -> val placa = b.arguments?.getString("placa") ?: ""; val id = b.arguments?.getLong("veiculoId") ?: 0L; val ctx = LocalContext.current; val db = remember { DatabaseHelper.get(ctx) }; val mm = remember { db.queryAll("frota").firstOrNull { db.str(it["placa"]) == placa }?.let { "${db.str(it["marca"])} ${db.str(it["modelo"])}".trim() } ?: "" }; CombustivelArlaMenuScreen(placa, mm, onCombustivel = { nav.navigate("abastecimento_hist/COMBUSTIVEL/$placa/$id") }, onArla = { nav.navigate("abastecimento_hist/ARLA/$placa/$id") }, onBack = { nav.popBackStack() }) }
                    composable("abastecimento_hist/{tipo}/{placa}/{veiculoId}") { b -> val tipo = if (b.arguments?.getString("tipo") == "ARLA") TipoAbastecimento.ARLA else TipoAbastecimento.COMBUSTIVEL; val placa = b.arguments?.getString("placa") ?: ""; val id = b.arguments?.getLong("veiculoId") ?: 0L; AbastecimentoHistoricoScreen(tipo, placa, id, onNovo = { nav.navigate("abastecimento_form/${tipo.name}/$placa/$id/-1") }, onEditar = { r -> nav.navigate("abastecimento_form/${tipo.name}/$placa/$id/$r") }, onBack = { nav.popBackStack() }) }
                    composable("abastecimento_form/{tipo}/{placa}/{veiculoId}/{registroId}") { b -> val tipo = if (b.arguments?.getString("tipo") == "ARLA") TipoAbastecimento.ARLA else TipoAbastecimento.COMBUSTIVEL; AbastecimentoFormScreen(tipo, b.arguments?.getString("placa") ?: "", b.arguments?.getLong("veiculoId") ?: 0L, b.arguments?.getLong("registroId") ?: -1) { nav.popBackStack() } }
