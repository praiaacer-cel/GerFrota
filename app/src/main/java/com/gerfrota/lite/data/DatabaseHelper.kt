...
class DatabaseHelper private constructor(ctx: Context) :
    ...
    // ✅ NOVAS FUNÇÕES PARA PNEUS
    fun updatePneuByCodigo(codigo: String, row: Map<String, Any?>): Int = writableDatabase.update(
        "pneus",
    ...
    fun valorAcumuladoPneu(codigo: String): Double {
        // Busca o pneu pelo código Fogo
        val pneu = queryAll("pneus").firstOrNull { str(it["codigo_fogo"]) == codigo } ?: return 0.0
        val pneuId = (pneu["id"] as? Long) ?: return 0.0
        
        // Valor de compra do pneu
        val valorCompra = num(pneu["valor_compra"])
        
        // Soma todos os custos dos serviços realizados neste pneu
        val servicos = queryAll("pneus_servicos").filter {
            (it["pneu_id"] as? Long) == pneuId
        }
        val custoServicos = servicos.sumOf { num(it["custo"]) }
        
        // Retorna o valor total acumulado (compra + serviços)
        return valorCompra + custoServicos
    }
